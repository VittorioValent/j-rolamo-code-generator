...
public class EntityGenerator {
    ...
    protected static void generateEntityClass(String entityName, List<FieldInfo> fields, Boolean auditable)
        ...
        FieldSpec id = FieldSpec.builder(Long.class, "id").addModifiers(Modifier.PRIVATE).addAnnotation(Id.class)
        ...
        for (FieldInfo fieldInfo : fields) {
            ...
            if (GeneratorUtils.getMappedEntity(fieldInfo.getType(), ModelType.ENTITY)) {
                fieldSpec = FieldSpec
                        .builder(GeneratorUtils.getFieldType(fieldInfo.getType(), ModelType.ENTITY),
                                fieldInfo.getName())
                        .addModifiers(Modifier.PRIVATE).addAnnotation(ManyToOne.class).addAnnotation(JoinColumn.class)
                        .build();
            }
            fieldSpecs.add(fieldSpec);
        }
        TypeSpec entityClass = TypeSpec.classBuilder(entityName).addModifiers(Modifier.PUBLIC)
                .addAnnotation(Entity.class).addAnnotation(Data.class).addAnnotation(Builder.class)
                ...
                .addField(id).addFields(fieldSpecs).superclass(GeneratorUtils.getEntitySuperClass(auditable))
