...
public class WorkshopService {
    ...
    private EntityInfo getEntityInfo(Class entityClass) {
        ...
        return EntityInfo.builder().auditable(auditable).entityName(entityName).fields(getDeclaredFields(entityClass))
                .build();
    }
    ...
    private List<FieldInfo> getDeclaredFields(Class entityClass) {
        ...
        for (Field field : declaredFields) {
            FieldInfo fieldInfo = FieldInfo.builder().name(field.getName()).type(field.getType().getTypeName()).build();
